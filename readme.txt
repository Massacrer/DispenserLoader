...
|	|
|	|---(in area mode)----------Empties all dispensers in the region selected by the /dload area command
|
|-------mode
|	|
|	|---chests------------------Sets the plugin to chest mode, which is as simple as it sounds. The area
|	|							mode command extensions to specify custom amounts and materials are especially
|	|							useful here.
|	|
|	|---dispensers--------------Sets the plugin back to its default mode. Note that this is also done by the
|								reset command
|
...
|	|							optimisation things :)
|	|
|	|---(in area mode)----------Fills up every dispenser in the selection region
|
